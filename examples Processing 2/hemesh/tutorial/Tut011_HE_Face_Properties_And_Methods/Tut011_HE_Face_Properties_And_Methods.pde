...
void setup() {
  ...
  WB_Point3d fc = face.getFaceCenter(); // face center

  int order=face.getFaceOrder();//number of vertices

  WB_Normal3d fn = face.getFaceNormal(); // face normal
  ...
  List<HE_Vertex> vertices = face.getFaceVertices(); // get all vertices belonging to face

  List<HE_Halfedge> halfedges = face.getFaceHalfedges();// get all halfedges belonging to face

  List<HE_Edge> edges = face.getFaceEdges();// get all edges belonging to face

  List<HE_Face> faces = face.getNeighborFaces();// get all neighboring faces
  ...
  List<WB_IndexedTriangle2D> tris = face.triangulate(); 
}
