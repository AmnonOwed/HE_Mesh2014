...
public class HeightCollision implements HeightEvent {
	 ...
	 * 
	 * Assumption is that there are no parallel edges creating horizontal
	 * bisectors in the current loops. We create some here, then process them
	 * all, again removing all horizontal bisectors from the current loops.
	 * 
	...
	public boolean process(Skeleton skel) {
		...
		/**
		 * todo: This is a two-step process, for (I suspect) historical reasons.
		 * It should be possible to find the chains as we go using
		 * line-projection.
		 */
	 ...
	 * Process horizontals.
	 * 
	 * assumes that all corners with horizontal bisectors are at the same
	...
	public void processHoriz(Skeleton skel) {
		...
		for (Chain chain : chains) {
			...
			// find a set of coplanar edges that survive this transition in
			...
			while (!priority.isEmpty()
					&& hComp.compare(winner, priority.get(0)) == 0)
				winners.add(priority.remove(0));

			// if first edge needs an additional corner -
			// "if we're adding a cap at the start" and
			// "first isn't already an interface"
			Corner first = chain.chain.get(0);
			if (!winners.contains(first.prevL)) // skel.liveCorners.contains(first)
			{
				if (!intraface.contains(first.prevC)) // hasn't already been
														// raised up by a
														// previous chain
				{
					// Tuple3d res =//new LinearForm3D( 0, 0, 1, -first.z
					// ).collide( first.prevL.linearForm,
					// first.prevC.prevL.linearForm );
					Corner newFirst = new Corner(Edge.collide(first.prevC,
							first.z));
					skel.output.addOutputSideTo(first.prevC, newFirst,
							first.prevL, first.prevC.prevL);
			...
			if (!winners.contains(last.nextL)) {
				if (!intraface.contains(last.nextC)) // hasn't already been
														// raised up by a
														// previous chain
				{
					// Tuple3d res = new LinearForm3D( 0, 0, 1, -last.z
					// ).collide( last.nextL.linearForm,
					// last.nextC.nextL.linearForm );
					Corner newLast = new Corner(
							Edge.collide(last.nextC, last.z));
					skel.output.addOutputSideTo(last.nextC, newLast,
							last.nextL, last.nextC.nextL);
			...
			for (Pair<Corner, Corner> pair : new ConsecutivePairs<Corner>(
				...
				if (winners.contains(s.nextL)) {
					if (s.nextL != winner) {
						skel.output.merge(winner.start, s); // assumes start of
															// edge forms part
															// of it's output
					}
