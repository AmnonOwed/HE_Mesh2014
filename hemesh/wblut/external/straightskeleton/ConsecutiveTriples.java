 ...
 * @author twak
 */
public class ConsecutiveTriples<E> implements Iterator<Triple<E, E, E>>,
		Iterable<Triple<E, E, E>> {
	List<E> input;
	int a = 0, size;
	boolean loop;
	 ...
	 * 
	 * @param input
	 *            the list to returns triples from
	 * @param loop
	...
	public Triple<E, E, E> next() {
		Triple<E, E, E> out = new Triple<E, E, E>(input.get(a % size),
				input.get((a + 1) % size), input.get((a + 2) % size));
	...
	// list.add( 4 );
	// list.add( 5 );
	// for ( Triple <Integer, Integer, Integer> t : new
	// ConsecutiveTriples<Integer>( list, false ))
	// {
