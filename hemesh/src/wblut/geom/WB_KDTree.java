...
public class WB_KDTree<T extends WB_Coordinate, V> {
	...
	public List<WB_AABB> getLeafBounds() {
		List<WB_AABB> leafs = new ArrayList();
		root.addLeafBounds(leafs);
		return leafs;
	}

	public List<WB_AABB> getAllBounds() {
		List<WB_AABB> all = new ArrayList();
		root.addBox(all, 0);
		return all;
	}
	...
	private class WB_KDNode<T extends WB_Coordinate, V> {
		...
		private V lookup(final T point) {
			for (int i = 0; i < _binSize; i++) {
				if (WB_Epsilon.isZeroSq(WB_Distance3D.sqDistance(point,
						_bin[i].coord))) {
					return _bin[i].value;
				}
			}
		...
		private void findNearest(final QueryResult<T, V> heap, final T data) {
			...
			if (_isLeaf) {
				for (int i = 0; i < _binSize; i++) {
					double dist = WB_Distance3D.sqDistance(_bin[i].coord, data);
		...
		private WB_KDEntry<T, V>[] range(final T center, final double r2) {
			...
			int n = 0;
			for (int i = 0; i < _binSize; i++) {
				double d2 = WB_Distance3D.sqDistance(center, _bin[i].coord);
				if (d2 <= r2) {
					_bin[i].d2 = d2;
					tmp[n++] = _bin[i];
				}
