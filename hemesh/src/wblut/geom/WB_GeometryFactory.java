...
public class WB_GeometryFactory {
	...
	public WB_Point createInterpolatedPoint(final WB_Coordinate p,
		...
		return new WB_Point((1.0 - f) * p.xd() + f * q.xd(), (1.0 - f) * p.yd()
				+ f * q.yd(), (1.0 - f) * p.zd() + f * q.zd());
	}
	...
	public WB_Point createInterpolatedPoint2D(final WB_Coordinate p,
			final WB_Coordinate q, final double f) {
		return createPoint((1.0 - f) * p.xd() + f * q.xd(), (1.0 - f) * p.yd()
				+ f * q.yd());
	}
	...
	public List<WB_Point> createIntersectionPoints(final WB_Circle C0,
		...
		final WB_Point c = createPoint(C0.getCenter())._addMulSelf(a / d, u);
	...
	public WB_Point createMirrorPoint(final WB_Coordinate p, final WB_Linear L) {
		...
		x2 = a * (p.xd() - L.getOrigin().xd()) + b
				* (p.yd() - L.getOrigin().yd()) + L.getOrigin().xd();
		y2 = b * (p.xd() - L.getOrigin().xd()) - a
				* (p.yd() - L.getOrigin().yd()) + L.getOrigin().yd();
	...
	public WB_Point createInversionPoint(final WB_Coordinate p,
		...
		final double OP = inversionCircle.getCenter().getDistance(p);
		if (WB_Epsilon.isZero(OP)) {
			return null;
		}
		final double OPp = r2 / OP;
		final WB_Vector v = createNormalizedVector(inversionCircle.getCenter(),
				p);
	...
	public WB_Point createPointFromTrilinearCoordinates(final double u,
		...
		final double eax = ((tri.p2().xd() - tri.p3().xd()) * bv + (tri.p1()
				.xd() - tri.p3().xd()) * au)
				* invabc + tri.p3().xd();
		final double eay = ((tri.p2().yd() - tri.p3().yd()) * bv + (tri.p1()
				.yd() - tri.p3().yd()) * au)
				* invabc + tri.p3().yd();
		final double eaz = ((tri.p2().zd() - tri.p3().zd()) * bv + (tri.p1()
				.zd() - tri.p3().zd()) * au)
				* invabc + tri.p3().zd();
	...
	public WB_Vector createVector(final WB_Coordinate p, final WB_Coordinate q) {
		return createVector(q.xd() - p.xd(), q.yd() - p.yd(), q.zd() - p.zd());
	}

	public WB_Vector createVector2D(final WB_Coordinate p, final WB_Coordinate q) {
	...
	public WB_Line createLineThroughPoints(final WB_Coordinate p1,
		...
		return createLineWithDirection(p1, createVector(p1, p2));
	}
	...
	public WB_Line createBisector2D(final WB_Coordinate p, final WB_Coordinate q) {
		return createLineWithDirection(
				createPoint(p)._mulAddMulSelf(0.5, 0.5, q),
	...
	public List<WB_Line> createLinesTangentToCircleThroughPoint(
		...
		final List<WB_Line> result = new ArrayList<WB_Line>(2);
		final double dcp = C.getCenter().getDistance(p);
		...
		if (WB_Epsilon.isZero(dcp - C.getRadius())) {
			...
			result.add(createLineWithDirection(
					p,
					createVector((r2 - (num + u.xd() * rad) / denom) / u.yd(),
							-((r2 * u.xd()) + rad) / denom)));
			result.add(createLineWithDirection(
					p,
					createVector((r2 - (num - u.xd() * rad) / denom) / u.yd(),
	...
	public List<WB_Line> createLinesTangentTo2Circles(final WB_Circle C0,
		...
		if (!WB_Epsilon.isZero(rdiff)) {
			...
			org = createPoint(C0.getCenter().xd() + s * w.xd(), C0.getCenter()
					.yd() + s * w.yd());
			if (!inside) {
				result.add(createLineWithDirection(org, dir.get(0)));
			}
	...
	public WB_Ray createRayThroughPoints(final WB_Coordinate p1,
			final WB_Coordinate p2) {
		return createRayWithDirection(p1, createVector(p2)._subSelf(p1));
	}
	...
	public WB_Segment createSegmentWithLength(final WB_Coordinate origin,
		...
		return createSegment(
				origin,
				createPoint(origin)._addMulSelf(length,
	...
	public WB_Circle createInversionCircle(final WB_Circle C,
			final WB_Circle inversionCircle) {
		if (WB_Classify.classifyPointToCircle2D(inversionCircle.getCenter(), C) == WB_Classification.ON) {
			return null;
		}
		...
		final double k2 = k * k;
		final double s = k2
				/ (C.getCenter().getSqDistance(inversionCircle.getCenter()) - C
	...
	public WB_Circle createCircumcircle(final WB_Triangle tri) {
		...
		final double radius = a
				* b
				* c
				/ Math.sqrt(2 * a * a * b * b + 2 * b * b * c * c + 2 * a * a
	...
	public WB_Circle createIncircle(final WB_Triangle tri) {
		...
		final double radius = 0.5 * Math.sqrt(((b + c - a) * (c + a - b) * (a
				+ b - c))
				* invabc);
		final double x = (tri.p1().xd() * a + tri.p2().xd() * b + tri.p3().xd()
				* c)
				* invabc;
		final double y = (tri.p1().yd() * a + tri.p2().yd() * b + tri.p3().yd()
				* c)
				* invabc;
		return createCircleWithRadius(createPoint(x, y), radius);
	}
	...
	public WB_Circle createCirclePPP(final WB_Coordinate p0,
		...
		return createCircleWithRadius(center, center.getDistance(p0));
	}
	...
	public List<WB_Circle> createCirclePPL(final WB_Coordinate p,
		...
		if (WB_Classify.sameSideOfLine2D(p, q, L) == WB_Classification.DIFF) {
			return result;
		}
		if (WB_Classify.classifyPointToLine2D(p, L) == WB_Classification.ON
				&& WB_Classify.classifyPointToLine2D(q, L) == WB_Classification.ON) {
			return result;
		}
		...
		if (WB_Classify.classifyPointToLine2D(p, L) == WB_Classification.ON) {
			if (WB_Epsilon.isZeroSq(createClosestPointOnLine2D(q, L)
				...
				result.add(createCircleWithRadius(
						...
						0.5 * WB_Distance.distanceToPoint2D(p, q)));
			} else {
				...
				result.add(createCircleWithRadius(intersect,
						WB_Distance.distanceToPoint2D(p, intersect)));
			}
			return result;
		}
		if (WB_Classify.classifyPointToLine2D(q, L) == WB_Classification.ON) {

			if (WB_Epsilon.isZeroSq(createClosestPointOnLine2D(p, L)
					.getSqDistance(q))) {
				result.add(createCircleWithRadius(createPoint(p)
						._mulAddMulSelf(0.5, 0.5, q), 0.5 * WB_Distance
						.distanceToPoint2D(p, q)));
			} else {
		...
		if (F == null) {
			final WB_Point r = createClosestPointOnLine2D(
					createPoint(0.5 * p.xd() + 0.5 * q.xd(), 0.5 * p.yd() + 0.5
							* q.yd()), L);
			result.add(createCirclePPP(p, q, r));
		} else {
			double d = WB_Distance.distanceToPoint2D(p, q);
	...
	public List<WB_Circle> createCirclePLL(final WB_Coordinate p,
		...
		if (A == null) {
			...
			for (final WB_Circle C : circles) {
				if (WB_Classify.sameSideOfLine2D(p, C.getCenter(), L1) == WB_Classification.SAME
						&& WB_Classify.sameSideOfLine2D(p, C.getCenter(), L2) == WB_Classification.SAME) {
					selcircles.add(C);
				}
			}
			...
			for (final WB_Circle C : selcircles) {
				...
				if (intersections.size() == 1) {
					...
					final double AG = A.getDistance(G);
					final double AD = A.getDistance(p);
					final double AE = A.getDistance(E);
					...
					final double AH = A.getDistance(H);
					final double AG = A.getDistance(G);
					final double AD = A.getDistance(p);
					final double AE = A.getDistance(E);
	...
	public List<WB_Circle> createCirclePPC(final WB_Coordinate p,
		...
		final WB_Classification pType = WB_Classify.classifyPointToCircle2D(p,
				C);
		final WB_Classification qType = WB_Classify.classifyPointToCircle2D(q,
				C);
		if (WB_Epsilon.isZero(WB_Distance.distanceToPoint2D(p, q))) {
			return result;
		}
		...
		if (pType == WB_Classification.ON && qType == WB_Classification.ON) {
			...
			if (center != null) {
				result.add(createCircleWithRadius(center, center.getDistance(p)));
			}
			return result;
		...
		// Both points outside
			...
			final WB_Circle iC2;
			final double k2;
			final boolean dp = WB_Epsilon.isZero(C.getCenter().getDistance(p));
			final boolean dq = WB_Epsilon.isZero(C.getCenter().getDistance(q));
			...
			if (dp || dq) {
				...
				final double s = k2
						/ (C.getCenter().getSqDistance(iC.getCenter()) - C
								.getRadius() * C.getRadius());
				iC2 = createCircleWithRadius(iC.getCenter().xd() + s
						* (C.getCenter().xd() - iC.getCenter().xd()), iC
						.getCenter().yd()
						+ s
			...
			for (final WB_Circle circle : iresult) {
				final double s = k2
						/ (circle.getCenter().getSqDistance(iC.getCenter()) - circle
								.getRadius() * circle.getRadius());
				result.add(createCircleWithRadius(iC.getCenter().xd() + s
						* (circle.getCenter().xd() - iC.getCenter().xd()), iC
						.getCenter().yd()
						+ s
						* (circle.getCenter().yd() - iC.getCenter().yd()),
	...
	public List<WB_Circle> createCirclePCC(final WB_Coordinate p,
		...
		// p on C1
		if (WB_Classify.classifyPointToCircle2D(p, C1) == WB_Classification.ON) {
			final WB_Line tangent = createLineTangentToCircleInPoint(C1, p);
			return createCirclePLC(p, tangent, C2);
		}

		// p on C2
		if (WB_Classify.classifyPointToCircle2D(p, C2) == WB_Classification.ON) {
			final WB_Line tangent = createLineTangentToCircleInPoint(C2, p);
			return createCirclePLC(p, tangent, C1);
		}

		final WB_Classification C1toC2 = WB_Classify.classifyCircleToCircle2D(
				C1, C2);
		...
		if (C1.isTangent(C2)) {
			...
			if (WB_Classify.classifyPointToCircle2D(p, C1) == WB_Classification.INSIDE) {
				return result;
			}
			final WB_Vector v = !WB_Epsilon.isZero(C1.getCenter().getDistance(
					C2.getCenter())) ? createNormalizedVector(C1.getCenter(),
					C2.getCenter()) : X();
			...
			final WB_Vector v = !WB_Epsilon.isZero(C1.getCenter().getDistance(
					C2.getCenter())) ? createNormalizedVector(C2.getCenter(),
					C1.getCenter()) : X();
		...
		// completely outside
				...
				|| (C1toC2 == WB_Classification.CROSSING && !(WB_Classify
						.classifyPointToCircle2D(p, C1) == WB_Classification.OUTSIDE)
						^ (WB_Classify.classifyPointToCircle2D(p, C2) == WB_Classification.OUTSIDE))) {
			...
			if (tangents.size() > 1) {
				...
				if (point != null) {
					...
					final List<WB_Point> intersections = createIntersectionPoints(
							Pp, circle);
					WB_Point Ep = null;
					if (!WB_Epsilon.isZero(intersections.get(0).getDistance(p))) {
		...
		// C1 and C2 crossing and p in only one of the two circles
			...
			if (WB_Classify.classifyPointToCircle2D(p, C1) == WB_Classification.INSIDE) {
				final double r1 = C1.getRadius()
						- C1.getCenter().getDistance(p);
				final double r2 = C2.getCenter().getDistance(p)
						- C2.getRadius();
				WB_Circle invC;
				if (r1 <= r2) {
					final WB_Vector v = createLineThroughPoints(C1.getCenter(),
							p).getDirection();
					final WB_Point center = createPoint(p)._addMulSelf(
							0.45 * r1, v);
	...
	public List<WB_Circle> createCirclePLC(final WB_Coordinate p,
		...
		if (WB_Classify.classifyPointToCircle2D(p, C) == WB_Classification.ON) {
			...
			if (WB_Classify.classifyPointToLine2D(p, Lperp) != WB_Classification.ON) {
				...
				if (WB_Epsilon.isZero(A1.getDistance(B))) {
	...
	public List<WB_Circle> createCircleLLL(final WB_Line L1, final WB_Line L2,
		...
		for (final WB_Point p : intersections) {
			...
			result.add(createCircleWithRadius(p, p.getDistance(p2)));
		}
	...
	public List<WB_Circle> createCircleTangentToLineThroughPoint(
		...
		if (WB_Epsilon.isZero(tmp2)) {
			...
			tmp2 = Math.sqrt(tmp2);
			final WB_Point tmpp = createPoint(p.xd() - a * tmp1, p.yd() - b
					* tmp1);
			result.add(createCircleWithRadius(
					createPoint(tmpp.xd() + tmp2 * b, tmpp.yd() - tmp2 * a), r));
			result.add(createCircleWithRadius(
					createPoint(tmpp.xd() - tmp2 * b, tmpp.yd() + tmp2 * a), r));
			return result;
		}
	...
	public List<WB_Circle> createCircleThroughPointTangentToCircle(
		...
		final List<WB_Circle> result = new ArrayList<WB_Circle>(4);
		final double dcp = createPoint(p).getDistance(C.getCenter());
	...
	public List<WB_Circle> createNonCollinearCircleTangentTo2Circles(
		...
		final List<WB_Circle> result = new ArrayList<WB_Circle>(2);
		final double d = C0.getCenter().getDistance(C1.getCenter());
		if (WB_Epsilon.isZero(d)) {
			return result;
		}
	...
	private List<WB_Circle> createCircleTangentTo3CollinearCircles(
			final WB_Circle C1, final WB_Circle C2, final WB_Circle C3) {
		final List<WB_Circle> result = new ArrayList<WB_Circle>();
		final WB_Circle third;
		if (C1.equals(C2) && C1.equals(C3)) {
			return result;
		}

		if (!C1.equals(C2)) {
			result.addAll(createCollinearCircleTangentTo2Circles(C1, C2));
			final Iterator<WB_Circle> itr = result.iterator();
			WB_Circle C;
			while (itr.hasNext()) {
				C = itr.next();
		...
		if (!C1.equals(C3)) {
			result.addAll(createCollinearCircleTangentTo2Circles(C1, C3));
			final Iterator<WB_Circle> itr = result.iterator();
			WB_Circle C;
			while (itr.hasNext()) {
				C = itr.next();
		...
		if (!C2.equals(C3)) {
			result.addAll(createCollinearCircleTangentTo2Circles(C2, C3));
			final Iterator<WB_Circle> itr = result.iterator();
			WB_Circle C;
			while (itr.hasNext()) {
				C = itr.next();
	...
	public WB_Plane createPlane(final WB_Coordinate p1, final WB_Coordinate p2,
		...
		final WB_Vector v21 = createVector(p1, p2);
		final WB_Vector v31 = createVector(p1, p3);
		return new WB_Plane(p1, v21._crossSelf(v31));

	}

	public WB_Plane createPlane(final Triangle T) {
	...
	public WB_Plane createOffsetPlane(final WB_Coordinate origin,
		...
		return new WB_Plane(createPoint(origin)._addMulSelf(offset, normal),
				normal);
	}
	...
	public WB_Plane createOffsetPlane(final double ox, final double oy,
			final double oz, final double nx, final double ny, final double nz,
			final double offset) {
		return new WB_Plane(createPoint(ox + offset * nx, oy + offset * ny, oz
				+ offset * nz), createVector(nx, ny, nz));
	}
	...
	public WB_Plane createOffsetPlane(final WB_Coordinate p1,
		...
		final WB_Vector v21 = createVector(p1, p2);
		final WB_Vector v31 = createVector(p1, p3);
		final WB_Vector n = v21._crossSelf(v31);
		n._normalizeSelf();
		return new WB_Plane(createPoint(p1)._addMulSelf(offset, n), n);

	}

	public WB_Mesh createMesh(final WB_Coordinate[] points, final int[][] faces) {
		return new WB_Mesh(points, faces);
	}

	public WB_Mesh createMesh(final Collection<? extends WB_Coordinate> points,
			final int[][] faces) {
		return new WB_Mesh(points, faces);
	}

	public WB_Mesh createMesh(final double[] ordinates, final int[][] faces) {
		return new WB_Mesh(ordinates, faces);
	}
	...
	public WB_Mesh createTriMesh(final WB_Coordinate[] points,
			final int[][] faces) {
		return new WB_TriMesh(points, faces);
	}

	public WB_Mesh createTriMesh(
			final Collection<? extends WB_Coordinate> points,
			final int[][] faces) {
		return new WB_TriMesh(points, faces);
	}
	...
	public WB_Mesh createConvexHull(final WB_Coordinate[] points,
	...
	public WB_Mesh createPrism(final WB_Coordinate[] points, final double h) {
		final WB_Vector offset = createVector(0, 0, h);
	...
	public WB_Mesh createPrism(final WB_Polygon poly, final double h) {
		...
		int numfaces = poly.getNumberOfPoints();
		final int[] nph = poly.getNumberOfPointsPerHole();
		for (int i = 0; i < poly.getNumberOfHoles(); i++) {
			numfaces += nph[i];
		}
		...
		for (int i = 0; i < poly.getNumberOfPoints(); i++) {
			...
			prismfaces[index][0] = 2 * i;
			prismfaces[index][1] = 2 * ((i + 1) % poly.getNumberOfPoints());
			prismfaces[index][2] = 2 * ((i + 1) % poly.getNumberOfPoints()) + 1;
			prismfaces[index][3] = 2 * i + 1;
			index++;
		}
		int count = poly.getNumberOfPoints();
		for (int i = 0; i < poly.getNumberOfHoles(); i++) {
			for (int j = 0; j < nph[i]; j++) {
				prismfaces[index] = new int[4];
				prismfaces[index][0] = 2 * (count + j);
				prismfaces[index][3] = 2 * (count + ((j + 1) % nph[i]));
				prismfaces[index][2] = 2 * (count + ((j + 1) % nph[i])) + 1;
				prismfaces[index][1] = 2 * (count + j) + 1;
				index++;
			}
			count += nph[i];
		}
	...
	public WB_Mesh createAntiprism(final int n, final double radius,
			final double h) {
	...
	public WB_Mesh createAntiPrism(
			final Collection<? extends WB_Coordinate> points, final double h) {
	...
	public WB_Mesh createAntiPrism(final WB_Coordinate[] points, final double h) {
	...
	public WB_Mesh createZonohedron(final WB_Coordinate[] vectors,
			final double scale) {
		...
		for (int i = 0; i < nop; i++) {
			...
			for (int p = 0; p < n; p++) {
				if (div % 2 == 0) {
					point._subSelf(vectors[p]);
				} else {
					point._addSelf(vectors[p]);
				}
				div = div / 2;
			}
			point._mulSelf(scale);
			points.add(point);
		}
	...
	public WB_Mesh createPyramidWithAngles(
			final Collection<? extends WB_Coordinate> points,
	...
	public WB_Mesh createPyramidWithAngles(final WB_Coordinate[] points,
			final double[] angles, final boolean b, final WB_Context2D context) {
	...
	public WB_Mesh createPyramidWithAngleRange(
			final Collection<? extends WB_Coordinate> points,
	...
	public WB_Mesh createPyramidWithAngleRange(final WB_Coordinate[] points,
			final double minangle, final double maxangle, final boolean b,
			final WB_Context2D context) {
	...
	public WB_Mesh createPyramidWithAngle(
			final Collection<? extends WB_Coordinate> points,
	...
	public WB_Mesh createPyramidWithAngle(final WB_Coordinate[] points,
			final double angle, final WB_Context2D context) {
		return createPyramidWithAngleRange(points, angle, angle, context);
	}
	...
	public WB_Mesh createPyramidWithHeight(
			final Collection<? extends WB_Coordinate> points,
	...
	public WB_Mesh createPyramidWithHeight(final WB_Coordinate[] points,
			final double height, final boolean b, final WB_Context2D context) {
	...
	public WB_Mesh createDipyramidWithAngleRange(final WB_Coordinate[] points,
			final double minangle, final double maxangle,
			final WB_Context2D context) {
	...
	public WB_Mesh createDipyramidWithAngle(final WB_Coordinate[] points,
			final double angle, final WB_Context2D context) {
		return createDipyramidWithAngleRange(points, angle, angle, context);
	}
	...
	public WB_Mesh createDipyramidWithHeight(final WB_Coordinate[] points,
			final double height) {
	...
	public WB_Mesh createTaperWithAnglesAndHeight(final WB_Coordinate[] points,
			final double angles[], final double height, final boolean b,
			final boolean t, final WB_Context2D context) {
		final Corner[] corners = new Corner[points.length];
		// final WB_Point local = createPoint();
	...
	public WB_Mesh createTaperWithAngleAndHeight(final WB_Coordinate[] points,
			final double angle, final double height, final WB_Context2D context) {
		return createTaperWithAngleRangeAndHeight(points, angle, angle, height,
				true, true, context);
		...
		for (final Face face : expfaces) {
			...
			for (final Loop<Point3d> faceloop : face.points) {
				...
				for (final Point3d p : faceloop) {
					tmp[i++] = counter;
					point = createPoint();
					context.pointTo3D(p.x, p.y, (p.z == 0) ? p.z + height * 0.5
							: +height * 0.5 + cap, point);
					lpoints.add(point);
			...
			for (final Loop<Point3d> faceloop : face.points) {
				...
				for (final Point3d p : faceloop) {
					tmp[faceloop.count() - 1 - i++] = counter;
					point = createPoint();
					context.pointTo3D(p.x, p.y, (p.z == 0) ? -p.z - height
							* 0.5 : -height * 0.5 - cap, point);
					lpoints.add(point);
					counter++;
				}
				tmpfaces.add(tmp);
	...
	WB_Mesh createSpindleWithAnglesAndHeight(final WB_Coordinate[] points,
			final double[] angles, final double height, final double cap,
			final WB_Context2D context) {
		final Corner[] corners = new Corner[points.length];
		// final WB_Point local = createPoint();
	...
	public WB_Mesh createSpindle(final WB_Coordinate[] points,
			final double height, final double cap, final WB_Context2D context) {
		final double[] angles = new double[points.length];
		for (int i = 0; i < points.length; i++) {
			angles[i] = 0.25 * Math.PI;
	...
	public WB_Mesh createConvexHull(
			final Collection<? extends WB_Coordinate> points) {
	...
	public WB_Mesh createConcaveHull(
	...
	public WB_Mesh createConcaveHull(final WB_AlphaComplex ac,
			final double filter) {
		return ac.getAlphaShapeMesh(filter);
	}

	public WB_Mesh createConcaveHull(final WB_Coordinate[] points,
			final double filter) {
	...
	public List<WB_Polygon> createText(final String text,
			final String fontName, final int pointSize) {

		return createText(text, new Font(fontName, 0, pointSize), 400.0);
	}

	public List<WB_Polygon> createText(final String text,
			final String fontName, final double flatness, final int pointSize) {
		return createText(text, new Font(fontName, 0, pointSize), flatness);
	}

	public List<WB_Polygon> createText(final String text,
			final String fontName, final int style, final double flatness,
			final int pointSize) {
		return createText(text, new Font(fontName, style, pointSize), flatness);
	}
	...
	public WB_Point createClosestPointOnTriangle(final WB_Coordinate p,
		...
		final WB_Vector ab = createVector(a, b);
		final WB_Vector ac = createVector(a, c);
		final WB_Vector ap = createVector(a, b);
		...
		final WB_Vector bp = createVector(b, p);
		...
		if (vc <= 0 && d1 >= 0 && d3 <= 0) {
			...
			return (createPoint(a)._addSelf(ab.mul(v)));
		}

		final WB_Vector cp = createVector(c, p);
		...
		if (vb <= 0 && d2 >= 0 && d6 <= 0) {
			...
			return (createPoint(a)._addSelf(ac.mul(w)));
		}

		final double va = d3 * d6 - d5 * d4;
		if (va <= 0 && (d4 - d3) >= 0 && (d5 - d6) >= 0) {
			final double w = (d4 - d3) / ((d4 - d3) + (d5 - d6));
			return (createPoint(b)._addSelf(createVector(c, b).mul(w)));
		}

		final double denom = 1.0 / (va + vb + vc);
		final double v = vb * denom;
		final double w = vc * denom;
		return (createPoint(a)._addSelf(ab.mul(v))._addSelf(ac.mul(w)));
	}
	...
	public double getDihedralAngle(final WB_Coordinate p1,
			final WB_Coordinate p2, final WB_Coordinate p3,
			final WB_Coordinate p4) {
		return getDihedralAngle(createVector(p2, p1), createVector(p3, p2),
				createVector(p4, p3));
	...
	public double getDihedralAngle(final WB_Coordinate v1,
		...
		final WB_Vector b2xb3 = createVector(v2)._crossSelf(v3);
		final WB_Vector b1xb2 = createVector(v1)._crossSelf(v2);
		final double x = b1xb2.dot(b2xb3);
		final double y = b1xb2._crossSelf(b2xb3)
				.dot(createNormalizedVector(v2));
	...
	public double getCosDihedralAngle(final WB_Coordinate p1,
			final WB_Coordinate p2, final WB_Coordinate p3,
			final WB_Coordinate p4) {
		return getCosDihedralAngle(createVector(p2, p1), createVector(p3, p2),
				createVector(p4, p3));
	...
	public double getCosDihedralAngle(final WB_Coordinate u,
		...
		final WB_Vector uv = createVector(u)._crossSelf(
				createVector(u)._addSelf(v));
		final WB_Vector vw = createVector(v)._crossSelf(
				createVector(v)._addSelf(w));
	...
	Polygon toJTSPolygon(final WB_Polygon poly) {
		Coordinate[] coords = new Coordinate[poly.getNumberOfPoints() + 1];
		int i = 0;
		for (i = 0; i < poly.getNumberOfPoints(); i++) {
		...
		int index = poly.getNumberOfPoints();
		for (i = 0; i < poly.getNumberOfHoles(); i++) {
			coords = new Coordinate[nph[i] + 1];
			coords[nph[i]] = toJTSCoordinate(poly.getPoint(index), index);
			for (int j = 0; j < nph[i]; j++) {
	...
	public WB_Point getIntersection(final WB_Coordinate a,
		...
		if (t >= -WB_Epsilon.EPSILON && t <= 1.0 + WB_Epsilon.EPSILON) {
			...
			return createPoint(a.xd() + t * (b.xd() - a.xd()),
					a.yd() + t * (b.yd() - a.yd()),
					a.zd() + t * (b.zd() - a.zd()));
