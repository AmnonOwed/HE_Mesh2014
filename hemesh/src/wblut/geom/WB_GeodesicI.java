...
public class WB_GeodesicI {
	...
	private void createMesh() {
		...
		if (div == EQUALCHORD) {
			...
			refpoints = new WB_Point[3 * (v - 1)];
			for (int i = 1; i < v; i++) {
				refpoints[i - 1 + 2 * (v - 1)] = gf.createPoint(WB_GeodesicMath
						.getPointOnGreatCircle(apices[0], apices[1], i * iv));
		...
		List<WB_Point> zeropoints = new FastTable<WB_Point>();
		points.add(apices[0]);
		for (int i = 0; i < v - 1; i++) {
		...
		case TETRAHEDRON:
			...
			break;
		case OCTAHEDRON:
			List<WB_Point> points4 = new FastTable<WB_Point>();
		...
		default:
			...
			List<WB_Point> points5 = new FastTable<WB_Point>();
			for (WB_Point p : zeropoints) {
			...
			List<WB_Point> points6 = new FastTable<WB_Point>();
			for (WB_Point p : zeropoints) {
			...
			T = new WB_Transform().addRotateY(-Math.PI).addRotateZ(
					-Math.PI / 180 * 144);
			List<WB_Point> points15 = new FastTable<WB_Point>();
			for (WB_Point p : zeropoints) {
			...
			for (WB_Point p : points5) {
				points.add(T.applyAsPoint(p));
			}
			for (WB_Point p : points6) {
				points.add(T.applyAsPoint(p));
			}
			for (WB_Point p : points15) {
				points.add(T.applyAsPoint(p));
			}
	...
	private WB_Point[][] midpoint(int log2v) {

		WB_Point[][] points = new WB_Point[v + 1][v + 1];
		points[0][0] = apices[0];
		points[v][0] = apices[1];
		points[v][v] = apices[2];
	...
	private void recdivtriangle(WB_Point[][] points, int i0, int j0, int i1,
			int j1, int i2, int j2, int r, int n) {
		int i01 = (i0 + i1) / 2;
		int j01 = (j0 + j1) / 2;
