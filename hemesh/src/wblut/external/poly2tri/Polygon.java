...
public class Polygon implements TriangleProvider {
	 ...
	 * @param p3
	 */
	public Polygon(final PolygonPoint p1, final PolygonPoint p2,
			final PolygonPoint p3) {
		p1._next = p2;
		p2._next = p3;
		p3._next = p1;
		p1._previous = p3;
	 ...
	 *
	 * @param points
	 *            - ordered list of points forming the polygon. No duplicates
	 *            are allowed
	 */
	 ...
	 * @param p
	 */
	public void insertPointAfter(final PolygonPoint a,
			final PolygonPoint newPoint) {
		// Validate that
	...
	public void prepareTriangulation(final TriangulationContext<?> tcx) {
		...
		if (_holes != null) {
			for (final Polygon p : _holes) {
				...
				tcx.newConstraint(p._points.get(0),
						p._points.get(p._points.size() - 1));
