 ...
 * 
 * note: the defn of weight here is a little strange, can alter it later as long
 * as the uphill vector calculation is updated too.
 * 
 * @author twak
 */
public class Edge {
	...
	// 0 is straight up, positive/-ve is inwards/outwards, absolute value must
	// be less than Math.PI/2
	...
	/**
	 * finds the Ax + By + Cz = D form of the edge Called when the the weight of
	 * the edge changes
	 */
	public void calculateLinearForm() {
		...
		linearForm = new LinearForm3D(norm, new Vector3d(start.x, start.y,
				start.z));
	}
	...
	/**
	 * Do these two edges go in the same direction and are they coliniear. (Are
	 * they parallel and go through the same point?)
	 */
	public boolean sameDirectedLine(Edge nextL) {
		return nextL.direction().angle(direction()) < 0.01
				&& Math.abs(getAngle() - nextL.getAngle()) < 0.01;
	}
	...
	public int hashCode() {
		/**
		 * Memo to self, we rely on the hashcode not changing when we edit the
		 * point's locations in wiggleui
		 */
	...
	public static Loop<Edge> fromPoints(List<Point2d> ribbon) {
		...
		for (Pair<Point2d, Point2d> pair : new ConsecutivePairs<Point2d>(
				ribbon, true)) {
			loop.append(new Edge(cache.get(pair.first()), cache.get(pair
					.second())));
		}
	...
	public static LoopL<Edge> fromPoints2d(LoopL<Point2d> ribbon) {
		...
		for (Loop<Point2d> pLoop : ribbon) {
			...
			for (Loopable<Point2d> pair : pLoop.loopableIterator())
				loop.append(new Edge(cache.get(pair.get()), cache.get(pair
						.getNext().get())));
	...
	/**
	 * 
	 * This is a robust collision of a's adjacent edges with a horizontal plane
	 * at the given height When Two parallel edges are given, we can assume that
	 * the
	 */
