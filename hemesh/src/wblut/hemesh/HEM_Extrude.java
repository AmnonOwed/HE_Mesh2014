...
public class HEM_Extrude extends HEM_Modifier {
	...
	private boolean applyStraightToOneFace(final int id,
		...
		final List<HE_Edge> newEdges = new FastTable<HE_Edge>();
	...
	private boolean applyFlatToOneFace(final int id,
		...
		if (isPossible) {
			...
			f.setLabel(1);
			final List<HE_Edge> newEdges = new FastTable<HE_Edge>();
			final List<HE_Edge> newEdges2 = new FastTable<HE_Edge>();
			he = f.getHalfedge();
			do {
				he = he.getNextInFace();
				newEdges.add(new HE_Edge());
			...
			final List<HE_Edge> edgesToRemove = new FastTable<HE_Edge>();
